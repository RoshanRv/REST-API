...
type ProductInputProp = Pick<
    ProductProp,
    "description" | "price" | "image" | "title"
>

const CreateModal = ({ showModal }: { showModal: (val: boolean) => void }) => {
    const {
        register,
        handleSubmit,
        formState: { errors },
    ...
    return (
        <div className="fixed top-0 left-0 bg-black/50 w-full h-full flex justify-center items-center z-50">
            <div className="bg-white p-10 rounded-lg shadow-lg shadow-purple-300 w-max  border-2 border-purple-500 relative">
                {/*    Close Btn      */}
                <button
